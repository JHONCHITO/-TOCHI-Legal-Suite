...
const jurisprudenciaOficial = [
  {
    titulo: "Corte Constitucional",
    subtitulo: "Relatoria oficial de sentencias C, T y SU",
  ...
  },
  {
    ...
    subtitulo: "Relatoria oficial de lo contencioso administrativo",
...
export default function LeyesPage() {
  ...
  const handleSearch = (event: FormEvent<HTMLFormElement>) => {
    ...
    if (!term) return;
    router.push(`/dashboard/busqueda?q=${encodeURIComponent(term)}`);
  };
        ...
        <TabsContent value="jurisprudencia" className="space-y-4">
          <Card>
            <CardHeader>
              ...
              <CardDescription>Acceso directo a relatorias oficiales y fuentes publicas.</CardDescription>
            </CardHeader>
            <CardContent>
              <div className="grid gap-3 md:grid-cols-3">
                {jurisprudenciaOficial.map((item) => (
                  <a
                    key={item.titulo}
                    href={item.href}
                    target="_blank"
