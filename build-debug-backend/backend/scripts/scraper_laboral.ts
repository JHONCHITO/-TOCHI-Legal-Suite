...
async function scrapeLaboral() {
  try {
    console.log("🚀 Scraping Código Laboral...");

    await dbConnect();
    console.log("✅ Conectado a MongoDB");

    const base = "https://leyes.co/codigo_sustantivo_del_trabajo/";

    const resultados: any[] = [];

    // 🔥 laboral ~500 artículos aprox
    for (let i = 1; i <= 600; i++) {
      try {
    ...
    await Norma.deleteMany({ codigo: "CL" });
    await Norma.insertMany(limpios);

    console.log("🔥 Código Laboral cargado");
